...
subset <- grep("6.2.2.+NRV", allOnGS$name)
allOnGS <- allOnGS[subset,]

multiFolder <- "multi/"
dir.create(multiFolder)
files <- by(allOnGS, simplify = FALSE, INDICES = seq(NROW(allOnGS)), function(d) { 
  st <- system.time({
    fileToDL <- file.path(multiFolder, d$name)
    out <- data.table(name = basename(fileToDL), local_path = fileToDL)
    if (!file.exists(fileToDL)) {
      out <- googledrive::drive_download(d, path = fileToDL)
    }
  }
  )
  print(st)
