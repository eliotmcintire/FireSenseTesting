...
inSim <- SpaDES.project::setupProject(
  ...
  .studyAreaName = {
    ifelse(exists(".studyAreaName"), .studyAreaName, paste0("ELF", .ELFind))
    },
  ...
  options = list(
    ...
    , reproducible.showSimilar = TRUE #interactive() && !nzchar(Sys.getenv("TMUX"))
  ...
  studyAreaLarge = {
    reproducible::prepInputs(url = 'https://drive.google.com/file/d/1gW6DBurw2uBx5cAZLcmWd6qBD7eMEd-4/view?usp=share_link',
      ...
      terra::buffer(width = -400000) |>
      terra::buffer(width = -30000) |> 
      terra::buffer(width = 30000)
  } 
...
# inSim$modules <- 
#   inSim$modules[grep("ELFs", names(inSim$modules), value = TRUE)]
